...
validation_predictions = np.load('xgb_advanced_NSCLC_4%_predictions.npy')
...
features = ['ALP', 'INR', 'D_D', 'CREA', 'MON_perc', 'BNP']
...
with st.sidebar:
    ...
    alp = st.number_input('Alkaline Phosphatase (ALP, U/L)', min_value=0.0, max_value=500.0, step=1, key='alp')
    inr = st.number_input('International Normalized Ratio (INR)', min_value=0.0, max_value=5.0, step=0.1, key='inr')
    d_d = st.number_input('D-dimer (D-D, mg/L)', min_value=0.0, max_value=10.0, step=0.1, key='d_d')
    crea = st.number_input('Creatinine (CREA, umol/L)', min_value=0.0, max_value=2000.0, step=1, key='crea')
    mon_perc = st.number_input('Percentage of monocytes (MON%)', min_value=0.0, max_value=100.0, step=1, key='mon_perc')
    bnp = st.number_input('B-type Natriuretic Peptide (BNP, pg/mL)', min_value=0.0, max_value=5000.0, step=1, key='bnp')
...
if st.button('Prediction'):
        # Prepare input data
        user_input = pd.DataFrame([[alp, inr, d_d, crea, mon_perc, bnp]], columns=features)
        

        # Normalize continuous features
        user_continuous_input_normalized = scaler.transform(user_input)
